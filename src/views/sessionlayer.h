...
class SessionLayer : public View {
    public:
        ...
        void Draw() override
        {
            if (IsTextFromModelUpdated()) LoadTextFromModel();
        ...
        bool IsTextFromModelUpdated()
        {
            SdfLayerHandle sessionLayer = GetModel()->GetSessionLayer();
            string layerText;
        ...
        TextEditor::Palette GetPalette()
        {
            ...
            ImU32 strongKeyCol =
                ImGui::GetColorU32(ImGuiCol_HeaderActive, 1.f);
        ...
        TextEditor::LanguageDefinition GetLanguageDefinitionUSD()
        {
            ...
            const char* const descriptors[] = {"add",     "append", "prepend",
                                               "del",     "delete", "custom",
                                               "uniform", "rel"};
            ...
            const char* const dataTypes[] = {
                "bool",       "uchar",      "int",        "int64",
                "uint",       "uint64",     "int2",       "int3",
                "int4",       "half",       "half2",      "half3",
                "half4",      "float",      "float2",     "float3",
                "float4",     "double",     "double2",    "double3",
                "double4",    "string",     "token",      "asset",
                "matrix2d",   "matrix3d",   "matrix4d",   "quatd",
                "quatf",      "quath",      "color3d",    "color3f",
                "color3h",    "color4d",    "color4f",    "color4h",
                "normal3d",   "normal3f",   "normal3h",   "point3d",
                "point3f",    "point3h",    "vector3d",   "vector3f",
                "vector3h",   "frame4d",    "texCoord2d", "texCoord2f",
                "texCoord2h", "texCoord3d", "texCoord3f", "texCoord3h"};

            const char* const schemas[] = {
                "Xform",  "Scope",    "Shader",         "Sphere",
                "Subdiv", "Camera",   "Cube",           "Curve",
                "Mesh",   "Material", "PointInstancer", "Plane"};

            const char* const specifiers[] = {"def", "over", "class",
                                              "variantSet"};
        ...
        void FocusInEvent() override { isEditing = true; };
